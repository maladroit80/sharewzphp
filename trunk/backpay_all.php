...
<div align="center">
    <table width="680" style="border-collapse: collapse" bordercolor="#111111" cellpadding="0" cellspacing="0">
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="5" style="border:1px solid #FFCC00;">
					   <tr align="center"><td colspan="5">返佣支付记录</td></tr>
...
$count=mysql_query("select count(*) from tb_backpay_history"); //获得记录总数
...
$result=mysql_query("select * from tb_backpay_history order by pay_time desc limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
while ($row=mysql_fetch_array($result)) {
	...
	<td style='background-color: #FAFAFA;text-align:center;'>". $row["pay_time"] ."</td></tr>";
...
if ($page != 1) { //页数不等于1
?>
<a href="backpay_all.php?page=<?php echo $page - 1;?>">上一页</a> <!--显示上一页-->
<?php
}
for ($i=1;$i<=$totalPage;$i++) {  //循环显示出页面
?>
<a href="backpay_all.php?page=<?php echo $i;?>"><?php echo $i ;?></a>
<?php
}
if ($page<$totalPage) { //如果page小于总页数,显示下一页链接
?>
<a href="backpay_all.php?page=<?php echo $page + 1;?>">下一页</a></td></tr>
<?php
} 
?>
					</table>
